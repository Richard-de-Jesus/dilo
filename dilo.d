...
void editorUpdateSyntax(erow* row)
{
    ...
    if (kilo.E.syntax == null)
        return; /* No syntax, everything is HL_NORMAL. */

    int i, prev_sep, in_string, in_comment;
    char* p;
    char** keywords = kilo.E.syntax.keywords;
    char* scs = kilo.E.syntax.singleline_comment_start.ptr;
    char* mcs = kilo.E.syntax.multiline_comment_start.ptr;
    char* mce = kilo.E.syntax.multiline_comment_end.ptr;
    ...
    if (row.idx > 0 && editorRowHasOpenComment(&kilo.E.row[row.idx - 1]))
        in_comment = 1;
    ...
    if (row.hl_oc != oc && row.idx + 1 < kilo.E.numrows)
        editorUpdateSyntax(&kilo.E.row[row.idx + 1]);
    row.hl_oc = oc;
}
