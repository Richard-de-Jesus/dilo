...
alias libc = core.stdc; // editorSave(
// editorOpen
...
mixin(LC!"ctype : isspace, isprint, isdigit;");
...
extern (C) void editorRefreshScreen()
{
    ...
    char[] status = sformat!("%.20s - %d lines %s")(status_buf,
        ED.filename, ED.numrows, ED.dirty ? "(modified)" : "");

    size_t len = status.length;

    char[] rstatus = sformat!("%d/%d")(rstatus_buf,
        ED.rowoff + ED.cy + 1, ED.numrows);
...
int editorOpen(string filename)
{
    ...
    {
        writeln(dstderr, "errno in editorOpen: ", errno);
        // note: this works because File constructors modify
        // errno when a exception is thrown.
        if (errno != ENOENT)
        {
            perror("Opening file");
            exit(1);
    ...
    {
        dstderr.writeln("unexpected exception type at", __LINE__);
        exit(1);
    }

    foreach(char[] line; lines(fp)) {
        editorInsertRow(ED.numrows, line, line.length);
    }
    
    // passing raw FILE* handle to getline.
    // char* line;
    // ssize_t linelen; size_t linecap;
    /*while ((linelen = getline(&line, &linecap, fp.getFP)) != -1)
    {
...
int editorRowHasOpenComment(ref erow row)
{
...
void editorUpdateSyntax(ref erow row)
{
...
void editorUpdateRow(ref erow row)
{
...
void editorInsertRow(int at, char[] s, size_t len)
{
    ...
    ED.row.length = ED.numrows + 1;
    
    if (at != ED.numrows)
    {
        // memmove(ED.row + at + 1, ED.row + at, ED.row[0].sizeof * (ED.numrows - at));
        moveArrays(ED.row[at + 1.. $], ED.row[at..$], ED.numrows - at);
    ...
    ED.row[at].chars = new char[len];
    ED.row[at].chars = s[0..len];
        //copyArrFromPtr(ED.row[at].chars, s, len);
...
{
    free(row.render);
    destroy(row.chars); // garbage collected.
    free(row.hl);
}
...
void editorDelRow(int at)
{
    ...
    moveArrays(ED.row[at..$], ED.row[at + 1..$], ED.numrows - at - 1);
...
char[] copyArrFromPtr(char[] dest, char* src, size_t len)
{
    foreach (i; 0 .. len)
    {
        dest[i] = *src;
...
void editorRowInsertChar(ref erow row, int at, int c)
{
    if (at > row.size)
    {
    ...
    else
    {
        ...
        row.chars.length++;
        moveArrays(row.chars[at + 1 .. $], row.chars[at..$], row.size - at + 1);
        row.size++;
    }
...
void moveArrays(T)(T[] dest, T[] src, size_t n)
{
    auto temp = src.dup;
    foreach (i; 0 .. n)
    {
...
inout(char)[] appendArrFromPtr(inout(char)[] dest, char* src, size_t n)
{
...
void editorRowAppendString(ref erow row, char[] s, size_t len)
{
    //row.chars = cast(char*) realloc(row.chars, row.size + len + 1);
    // row.chars = appendArrFromPtr(row.chars, s, len); // version 2
    row.chars ~= s;
...
void editorRowDelChar(ref erow row, int at)
{
    if (row.size <= at)
        return;

    moveArrays(row.chars[at .. $], row.chars[at + 1 .. $], row.size - at);
...
void editorInsertChar(int c)
{
    ...
    if (!row)
    {
        while (ED.numrows <= filerow)
            editorInsertRow(ED.numrows, "".dup, 0);
    }
    
    ref erow valid_row = ED.row[filerow];
    editorRowInsertChar(valid_row, filecol, c);
...
void editorInsertNewline()
{
    int filerow = ED.rowoff + ED.cy;
    int filecol = ED.coloff + ED.cx;
    erow* optional_row = (filerow >= ED.numrows) ? null : &ED.row[filerow];

    if (!optional_row)
    {
        if (filerow == ED.numrows)
        {
            editorInsertRow(filerow, "".dup, 0);
        }
        else
        {
            return;
    ...
    {
        // safe to cast, now we know its not null
        ref erow row = *optional_row;
...
void editorDelChar()
{
    ...
    {
        // safe, we know it is not null
        ref erow valid_row = *row;
        editorRowDelChar(valid_row, filecol - 1);
...
int editorSave()
{
    ...
    try
    {
        toFile(buf, ED.filename);
        ED.dirty = 0;
        // trick, so that the previous message
        // is not merged with new one
        char[70] blank = 32;
        editorSetStatusMessage(blank);

        editorSetStatusMessage("%s bytes written on disk", buf.length);
        return 0;
    }
    catch (Exception ex)
    {
...
{
    char* _new = cast(char*) realloc(ab.b, cast(int)(ab.len + len));
